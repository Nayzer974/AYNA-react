-- ============================================
-- NOUVEAU BACKEND DHIKR POUR APPLICATION MOBILE
-- ============================================
-- Backend propre et simple optimisé pour mobile
-- Fonctionne avec les utilisateurs connectés (même sans email vérifié)
-- ============================================

-- ============================================
-- ÉTAPE 1 : Créer les tables
-- ============================================

-- Table principale pour les sessions de dhikr
CREATE TABLE IF NOT EXISTS public.dhikr_sessions (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  created_by UUID REFERENCES auth.users(id) ON DELETE CASCADE NOT NULL,
  dhikr_text TEXT NOT NULL,
  target_count INTEGER NOT NULL CHECK (target_count >= 100 AND target_count <= 999),
  current_count INTEGER DEFAULT 0 NOT NULL CHECK (current_count >= 0),
  is_active BOOLEAN DEFAULT true NOT NULL,
  is_open BOOLEAN DEFAULT true NOT NULL,
  max_participants INTEGER DEFAULT 100 NOT NULL CHECK (max_participants > 0 AND max_participants <= 100),
  created_at TIMESTAMPTZ DEFAULT now() NOT NULL,
  updated_at TIMESTAMPTZ DEFAULT now() NOT NULL,
  completed_at TIMESTAMPTZ
);

-- Table pour les participants des sessions
CREATE TABLE IF NOT EXISTS public.dhikr_session_participants (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  session_id UUID REFERENCES public.dhikr_sessions(id) ON DELETE CASCADE NOT NULL,
  user_id UUID REFERENCES auth.users(id) ON DELETE SET NULL, -- NULL autorisé pour les utilisateurs non authentifiés
  user_name TEXT,
  user_email TEXT,
  joined_at TIMESTAMPTZ DEFAULT now() NOT NULL,
  click_count INTEGER DEFAULT 0 NOT NULL CHECK (click_count >= 0),
  UNIQUE (session_id, user_id)
);

-- Table pour gérer les clics (optionnelle, pour statistiques futures)
CREATE TABLE IF NOT EXISTS public.dhikr_session_clicks (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  session_id UUID REFERENCES public.dhikr_sessions(id) ON DELETE CASCADE NOT NULL,
  user_id UUID REFERENCES auth.users(id) ON DELETE SET NULL, -- NULL autorisé
  clicked_at TIMESTAMPTZ DEFAULT now() NOT NULL,
  processed_at TIMESTAMPTZ,
  processed BOOLEAN DEFAULT false NOT NULL
);

-- ============================================
-- ÉTAPE 2 : Créer les index pour les performances
-- ============================================

-- Index pour les sessions actives (requête la plus fréquente)
CREATE INDEX IF NOT EXISTS idx_dhikr_sessions_active 
  ON public.dhikr_sessions (is_active, is_open, created_at DESC) 
  WHERE is_active = true AND is_open = true;

-- Index pour trouver les sessions d'un utilisateur
CREATE INDEX IF NOT EXISTS idx_dhikr_sessions_created_by 
  ON public.dhikr_sessions (created_by);

-- Index pour les participants par session
CREATE INDEX IF NOT EXISTS idx_dhikr_session_participants_session 
  ON public.dhikr_session_participants (session_id);

-- Index pour trouver les sessions d'un utilisateur (via participants)
CREATE INDEX IF NOT EXISTS idx_dhikr_session_participants_user 
  ON public.dhikr_session_participants (user_id) 
  WHERE user_id IS NOT NULL;

-- Index pour les clics non traités
CREATE INDEX IF NOT EXISTS idx_dhikr_session_clicks_pending 
  ON public.dhikr_session_clicks (session_id, processed) 
  WHERE processed = false;

-- ============================================
-- ÉTAPE 3 : Créer la fonction de trigger pour updated_at
-- ============================================

CREATE OR REPLACE FUNCTION public.update_dhikr_sessions_updated_at()
RETURNS TRIGGER 
LANGUAGE plpgsql
AS $$
BEGIN
  NEW.updated_at = now();
  RETURN NEW;
END;
$$;

-- Créer le trigger
DROP TRIGGER IF EXISTS trigger_update_dhikr_sessions_updated_at ON public.dhikr_sessions;
CREATE TRIGGER trigger_update_dhikr_sessions_updated_at
  BEFORE UPDATE ON public.dhikr_sessions
  FOR EACH ROW
  EXECUTE FUNCTION public.update_dhikr_sessions_updated_at();

-- ============================================
-- ÉTAPE 4 : Configurer Row Level Security (RLS)
-- ============================================

-- Activer RLS
ALTER TABLE public.dhikr_sessions ENABLE ROW LEVEL SECURITY;
ALTER TABLE public.dhikr_session_participants ENABLE ROW LEVEL SECURITY;
ALTER TABLE public.dhikr_session_clicks ENABLE ROW LEVEL SECURITY;

-- ============================================
-- POLITIQUES RLS POUR dhikr_sessions
-- ============================================

-- Tout le monde peut voir les sessions actives
CREATE POLICY "Anyone can view active sessions"
  ON public.dhikr_sessions
  FOR SELECT
  USING (is_active = true AND is_open = true);

-- Les utilisateurs connectés peuvent créer des sessions
-- On utilise getSession() côté client, donc auth.uid() peut être NULL
-- mais on vérifie dans la fonction RPC que l'utilisateur existe
CREATE POLICY "Authenticated users can create sessions"
  ON public.dhikr_sessions
  FOR INSERT
  WITH CHECK (auth.uid() IS NOT NULL);

-- Les créateurs peuvent mettre à jour leurs sessions
CREATE POLICY "Creators can update their sessions"
  ON public.dhikr_sessions
  FOR UPDATE
  USING (auth.uid() = created_by)
  WITH CHECK (auth.uid() = created_by);

-- Les créateurs peuvent supprimer leurs sessions
CREATE POLICY "Creators can delete their sessions"
  ON public.dhikr_sessions
  FOR DELETE
  USING (auth.uid() = created_by);

-- ============================================
-- POLITIQUES RLS POUR dhikr_session_participants
-- ============================================

-- Tout le monde peut voir les participants
CREATE POLICY "Anyone can view participants"
  ON public.dhikr_session_participants
  FOR SELECT
  USING (true);

-- Les utilisateurs connectés peuvent rejoindre des sessions
CREATE POLICY "Authenticated users can join sessions"
  ON public.dhikr_session_participants
  FOR INSERT
  WITH CHECK (auth.uid() IS NOT NULL OR user_id IS NULL);

-- Les utilisateurs peuvent quitter leurs sessions
CREATE POLICY "Users can leave sessions"
  ON public.dhikr_session_participants
  FOR DELETE
  USING (auth.uid() = user_id OR user_id IS NULL);

-- Les utilisateurs peuvent mettre à jour leur participation
CREATE POLICY "Users can update their participation"
  ON public.dhikr_session_participants
  FOR UPDATE
  USING (auth.uid() = user_id)
  WITH CHECK (auth.uid() = user_id);

-- ============================================
-- POLITIQUES RLS POUR dhikr_session_clicks
-- ============================================

-- Tout le monde peut voir les clics (pour statistiques)
CREATE POLICY "Anyone can view clicks"
  ON public.dhikr_session_clicks
  FOR SELECT
  USING (true);

-- Les utilisateurs connectés peuvent ajouter des clics
CREATE POLICY "Authenticated users can add clicks"
  ON public.dhikr_session_clicks
  FOR INSERT
  WITH CHECK (auth.uid() IS NOT NULL OR user_id IS NULL);

-- ============================================
-- ÉTAPE 5 : Créer les fonctions RPC
-- ============================================

-- Fonction principale pour créer une session de dhikr
-- Utilise SECURITY DEFINER pour contourner RLS et vérifie manuellement l'utilisateur
CREATE OR REPLACE FUNCTION public.create_dhikr_session(
  p_user_id UUID,
  p_dhikr_text TEXT,
  p_target_count INTEGER,
  p_max_participants INTEGER DEFAULT 100
)
RETURNS UUID
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public, pg_temp
AS $$
DECLARE
  v_session_id UUID;
  v_existing_session_id UUID;
  v_user_exists BOOLEAN;
  v_user_name TEXT;
  v_user_email TEXT;
BEGIN
  -- Vérifier que l'utilisateur existe dans auth.users
  SELECT EXISTS(SELECT 1 FROM auth.users WHERE id = p_user_id)
  INTO v_user_exists;
  
  IF NOT v_user_exists THEN
    RAISE EXCEPTION 'L''utilisateur n''existe pas dans la base de données. Veuillez vous reconnecter.';
  END IF;

  -- Vérifier si l'utilisateur est déjà dans une session active
  SELECT session_id
  INTO v_existing_session_id
  FROM public.dhikr_session_participants
  WHERE user_id = p_user_id
    AND EXISTS (
      SELECT 1 FROM public.dhikr_sessions
      WHERE id = session_id AND is_active = true
    )
  LIMIT 1;

  IF v_existing_session_id IS NOT NULL THEN
    RAISE EXCEPTION 'Vous êtes déjà dans une autre session. Veuillez quitter votre session actuelle avant d''en créer une nouvelle.';
  END IF;

  -- Valider les paramètres
  IF p_target_count < 100 OR p_target_count > 999 THEN
    RAISE EXCEPTION 'Le nombre de clics doit être entre 100 et 999';
  END IF;

  IF p_max_participants < 1 OR p_max_participants > 100 THEN
    RAISE EXCEPTION 'Le nombre maximum de participants doit être entre 1 et 100';
  END IF;

  -- Créer la session
  INSERT INTO public.dhikr_sessions (
    created_by,
    dhikr_text,
    target_count,
    max_participants
  )
  VALUES (
    p_user_id,
    p_dhikr_text,
    p_target_count,
    p_max_participants
  )
  RETURNING id INTO v_session_id;

  -- Récupérer les informations de l'utilisateur
  SELECT 
    COALESCE(raw_user_meta_data->>'name', email, 'Utilisateur'),
    email
  INTO v_user_name, v_user_email
  FROM auth.users
  WHERE id = p_user_id;

  -- Ajouter le créateur comme participant
  INSERT INTO public.dhikr_session_participants (
    session_id,
    user_id,
    user_name,
    user_email
  )
  VALUES (
    v_session_id,
    p_user_id,
    v_user_name,
    v_user_email
  )
  ON CONFLICT (session_id, user_id) DO NOTHING;

  RETURN v_session_id;
END;
$$;

-- Fonction pour rejoindre une session
CREATE OR REPLACE FUNCTION public.join_dhikr_session(
  p_user_id UUID,
  p_session_id UUID
)
RETURNS BOOLEAN
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public, pg_temp
AS $$
DECLARE
  v_user_exists BOOLEAN;
  v_participant_count INTEGER;
  v_max_participants INTEGER;
  v_is_open BOOLEAN;
  v_is_active BOOLEAN;
  v_user_name TEXT;
  v_user_email TEXT;
BEGIN
  -- Vérifier que l'utilisateur existe
  IF p_user_id IS NOT NULL THEN
    SELECT EXISTS(SELECT 1 FROM auth.users WHERE id = p_user_id)
    INTO v_user_exists;
    
    IF NOT v_user_exists THEN
      RAISE EXCEPTION 'L''utilisateur n''existe pas dans la base de données.';
    END IF;
  END IF;

  -- Vérifier que la session existe et est ouverte
  SELECT 
    is_open,
    is_active,
    max_participants,
    (SELECT COUNT(*) FROM public.dhikr_session_participants WHERE session_id = p_session_id)
  INTO 
    v_is_open,
    v_is_active,
    v_max_participants,
    v_participant_count
  FROM public.dhikr_sessions
  WHERE id = p_session_id;

  IF NOT FOUND THEN
    RAISE EXCEPTION 'La session n''existe pas.';
  END IF;

  IF NOT v_is_open THEN
    RAISE EXCEPTION 'La session n''est plus ouverte aux nouveaux participants.';
  END IF;

  IF NOT v_is_active THEN
    RAISE EXCEPTION 'La session n''est plus active.';
  END IF;

  IF v_participant_count >= v_max_participants THEN
    RAISE EXCEPTION 'La session est pleine.';
  END IF;

  -- Récupérer les informations de l'utilisateur
  IF p_user_id IS NOT NULL THEN
    SELECT 
      COALESCE(raw_user_meta_data->>'name', email, 'Utilisateur'),
      email
    INTO v_user_name, v_user_email
    FROM auth.users
    WHERE id = p_user_id;
  END IF;

  -- Ajouter le participant
  INSERT INTO public.dhikr_session_participants (
    session_id,
    user_id,
    user_name,
    user_email
  )
  VALUES (
    p_session_id,
    p_user_id,
    v_user_name,
    v_user_email
  )
  ON CONFLICT (session_id, user_id) DO NOTHING;

  RETURN true;
END;
$$;

-- Fonction pour quitter une session
CREATE OR REPLACE FUNCTION public.leave_dhikr_session(
  p_user_id UUID,
  p_session_id UUID
)
RETURNS BOOLEAN
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public, pg_temp
AS $$
DECLARE
  v_remaining_participants INTEGER;
  v_created_by UUID;
BEGIN
  -- Supprimer le participant
  DELETE FROM public.dhikr_session_participants
  WHERE session_id = p_session_id 
    AND (user_id = p_user_id OR (p_user_id IS NULL AND user_id IS NULL));

  -- Compter les participants restants
  SELECT COUNT(*)
  INTO v_remaining_participants
  FROM public.dhikr_session_participants
  WHERE session_id = p_session_id;

  -- Récupérer le créateur de la session
  SELECT created_by
  INTO v_created_by
  FROM public.dhikr_sessions
  WHERE id = p_session_id;

  -- Si plus aucun participant, supprimer la session
  IF v_remaining_participants = 0 THEN
    DELETE FROM public.dhikr_sessions
    WHERE id = p_session_id;
  -- Si c'est le créateur qui quitte mais qu'il reste des participants, fermer la session
  ELSIF p_user_id = v_created_by THEN
    UPDATE public.dhikr_sessions
    SET is_active = false, is_open = false
    WHERE id = p_session_id;
  END IF;

  RETURN true;
END;
$$;

-- ============================================
-- ÉTAPE 6 : Donner les permissions
-- ============================================

-- Permettre à tous les utilisateurs authentifiés d'exécuter les fonctions
GRANT EXECUTE ON FUNCTION public.create_dhikr_session(UUID, TEXT, INTEGER, INTEGER) TO authenticated;
GRANT EXECUTE ON FUNCTION public.join_dhikr_session(UUID, UUID) TO authenticated;
GRANT EXECUTE ON FUNCTION public.leave_dhikr_session(UUID, UUID) TO authenticated;

-- Permettre à tous les utilisateurs de voir les données (via RLS)
GRANT SELECT ON public.dhikr_sessions TO authenticated, anon;
GRANT SELECT ON public.dhikr_session_participants TO authenticated, anon;
GRANT SELECT ON public.dhikr_session_clicks TO authenticated, anon;

-- Permettre aux utilisateurs authentifiés d'insérer/mettre à jour
GRANT INSERT, UPDATE, DELETE ON public.dhikr_sessions TO authenticated;
GRANT INSERT, UPDATE, DELETE ON public.dhikr_session_participants TO authenticated;
GRANT INSERT ON public.dhikr_session_clicks TO authenticated;

-- ============================================
-- ✅ BACKEND CRÉÉ AVEC SUCCÈS
-- ============================================
-- Le backend est maintenant prêt pour l'application mobile
-- Les utilisateurs connectés (même sans email vérifié) peuvent créer des sessions

