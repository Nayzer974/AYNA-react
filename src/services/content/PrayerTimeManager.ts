...
// import * as Notifications from 'expo-notifications'; // DÉSACTIVÉ
import * as Location from 'expo-location';
import Constants from 'expo-constants';
// import { requestNotificationPermissions } from './notifications'; // DÉSACTIVÉ
...
async function fetchTodayPrayerTimes(
  ...
  try {
    const cachedDataStr = await storage.getItem(CACHE_KEY);
    if (cachedDataStr) {
      const cachedData: CachedPrayerTimes = JSON.parse(cachedDataStr);
      const now = new Date();
      const today = now.toISOString().split('T')[0];

      // Vérifier si le cache est valide pour aujourd'hui, la localisation (à ~10km près) et la méthode
      if (
        cachedData.date === today &&
        cachedData.method === method &&
        Math.abs(cachedData.latitude - latitude) < 0.1 &&
        Math.abs(cachedData.longitude - longitude) < 0.1
      ) {
        console.log('[PrayerTimeManager] Utilisation des horaires en cache');
        return cachedData.timings;
      }
    }
  } catch (e) {
    console.error('[PrayerTimeManager] Erreur lecture cache:', e);
  }
  ...
  try {
    ...
    console.error('[PrayerTimeManager] Erreur écriture cache:', e);
  }
...
async function scheduleNotifications(futurePrayers: FuturePrayer[]): Promise<void> {
  ...
  for (const prayer of futurePrayers) {
    ...
    for (const moment of notificationMoments) {
      ...
      // DÉSACTIVÉ - Les notifications Expo ont été retirées
      // try {
...
export async function initialize(userLocation?: LocationCoords): Promise<void> {
  try {
    ...
    // if (!hasPermission) {
    //   console.log('[PrayerTimeManager] Permissions de notification refusées');
    //   return;
    // }
    //
    ...
    // ÉTAPE 3 : Planification - DÉSACTIVÉ
    // Pour chaque prière future, planifier 3 notifications (10min, 5min, à l'heure)
    // await scheduleNotifications(futurePrayers); // DÉSACTIVÉ
