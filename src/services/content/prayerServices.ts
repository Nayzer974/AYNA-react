...
export async function getPrayerTimesByCoords(
  ...
  // Créer une clé cache unique pour aujourd'hui et ces coordonnées (arrondies)
  const now = new Date();
  const today = now.toISOString().split('T')[0];
  const latKey = latitude.toFixed(2);
  const lonKey = longitude.toFixed(2);
  const cacheKey = `HIJRI_PRAYER_TIMES_${today}_${latKey}_${lonKey}_${method}`;

  // Vérifier le cache
  try {
    const cached = await storage.getItem(cacheKey);
    if (cached) {
      console.log('[hijri.ts] Cache hit pour getPrayerTimesByCoords');
      return JSON.parse(cached);
    }
  } catch (e) {
    console.error('Erreur lecture cache hijri:', e);
  }
  ...
  if (res.status === 429) {
    console.warn('[hijri.ts] Erreur 429 Aladhan API, tentative de récupération du cache le plus récent...');
    // TODO: Implémenter une stratégie de fallback plus robuste si nécessaire
  }
  ...
  try {
    ...
    console.error('Erreur écriture cache hijri:', e);
  }
