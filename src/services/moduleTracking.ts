// Service de tracking des modules avec la règle des 10 secondes
...
/**
 * Enregistre une visite validée d'un module (après 10 secondes)
 * Sauvegarde toujours localement, puis synchronise vers Supabase si en ligne
 * 
 * @param moduleName - Le nom du module visité
 */
export async function logModuleVisit(moduleName: TrackedModule): Promise<void> {
  try {
    ...
    // Vérifier si on est en ligne et si Supabase est configuré
    const online = await isOnline();
    
    if (!online || !APP_CONFIG.useSupabase || !supabase) {
      // Hors ligne : ajouter à la queue de synchronisation si utilisateur connecté
      try {
        if (supabase) {
        ...
        // Ignorer les erreurs si pas d'utilisateur connecté
      }
      return;
    }
    ...
    try {
      ...
      // Si erreur, ajouter à la queue pour synchronisation ultérieure
      try {
        const { data: { user } } = await supabase.auth.getUser();
        if (user) {
          await addToSyncQueue({
...
async function saveModuleVisitLocal(visit: ModuleVisit): Promise<void> {
  try {
    ...
    // Limiter à 10000 visites pour éviter de saturer le stockage
...
async function saveModuleVisitRemote(userId: string, visit: ModuleVisit): Promise<void> {
  ...
  try {
    // Vérifier si la table existe, sinon créer une entrée dans user_metadata
...
async function saveModuleVisitToUserMetadata(userId: string, visit: ModuleVisit): Promise<void> {
  ...
  try {
    ...
    const updatedVisits = [
      ...
      ...moduleVisits
    ].slice(0, 1000); // Limiter à 1000 dans les métadonnées
...
/**
 * Filtre les visites selon une période donnée
 * 
 * @param visits - Liste des visites
 * @param periodHours - Nombre d'heures à remonter (24, 168 pour 7 jours, 720 pour 30 jours)
 */
...
/**
 * Calcule la fréquence d'utilisation par module pour une période donnée
 * 
 * @param visits - Liste des visites filtrées
...
export async function loadAllModuleVisits(userId?: string): Promise<ModuleVisit[]> {
  ...
  if (userId && APP_CONFIG.useSupabase && supabase) {
    try {
      ...
      // Fusionner les deux sources en évitant les doublons (basé sur timestamp + moduleId)
      ...
      // Ajouter les visites locales (elles peuvent être plus récentes)
