...
async function saveWidgetDataForNative(data: AllWidgetsData): Promise<void> {
  try {
    // Pour Android : Utiliser notre module native bridge
    if (Platform.OS === 'android' && NativeModules.AynaWidgetModule) {
      if (data.prayerTimes) {
        await NativeModules.AynaWidgetModule.setWidgetData('prayer_times', JSON.stringify(data.prayerTimes));
      }
      if (data.dhikr) {
        await NativeModules.AynaWidgetModule.setWidgetData('dhikr', JSON.stringify(data.dhikr));
      }
      if (data.verse) {
        await NativeModules.AynaWidgetModule.setWidgetData('verse', JSON.stringify(data.verse));
      }
      return;
    }

    // Pour iOS : App Groups (nécessitera un module similaire)

    // Fallback AsyncStorage (pour compatibilité)
    if (data.prayerTimes) {
