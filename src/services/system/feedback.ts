/**
 * Service pour gÃ©rer l'envoi de feedbacks (avis/bugs) avec images
 */
...
export interface FeedbackData {
  ...
  subject: string;
  message: string;
  imageUris?: string[]; // URIs locales des images Ã  uploader
}
  ...
  try {
    ...
    if (feedbackData.imageUris && feedbackData.imageUris.length > 0 && userId) {
      try {
        ...
        // Continuer mÃªme si l'upload d'images Ã©choue
        return {
          success: false,
