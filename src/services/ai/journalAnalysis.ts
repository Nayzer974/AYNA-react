...
import { loadJournalNotes, type JournalNote } from '@/services/storage/notesStorage';
...
export async function analyzeJournal(
  ...
  try {
    ...
    const filteredNotes = notes.filter(note => {
      ...
      return timeDiff >= 0 && timeDiff <= timeRanges[timeRange];
    });
    ...
    // ✅ OPTIMISÉ : Ne pas bloquer le thread JS pendant l'analyse lourde
    const emotionAnalysis = await new Promise<EmotionAnalysis>(resolve => {
      InteractionManager.runAfterInteractions(() => {
        resolve(analyzeEmotions(sortedNotes));
      });
    });
...
Clearly identify emotional evolution over time.`}`;
    ...
    try {
      ...
      // Extraction plus robuste du JSON (trouver la première { et la dernière })
      const firstOpen = cleanedContent.indexOf('{');
      const lastClose = cleanedContent.lastIndexOf('}');

      if (firstOpen !== -1 && lastClose !== -1 && lastClose > firstOpen) {
        const jsonString = cleanedContent.substring(firstOpen, lastClose + 1);
        ...
        console.warn('[journalAnalysis] Aucun JSON valide trouvé dans la réponse');
      }
    } catch (parseError) {
    ...
    // Gérer l'erreur de rate limit Ollama
    if (error.message === 'OLLAMA_RATE_LIMIT' || error.isOllamaRateLimit || error.status === 429) {
      return {
        summary: 'L\'IA est temporairement surchargée. Veuillez réessayer dans quelques instants.',
        themes: [],
        patterns: [],
        recommendations: [],
        emotions: {
...
export async function analyzeSingleNote(
  ...
  try {
    if (!noteText || noteText.trim().length === 0) {
      console.warn('[journalAnalysis] analyzeSingleNote: noteText est vide');
      return {
        emotions: [],
        sentiment: 'neutral',
        themes: [],
        insight: 'La note est vide, impossible de l\'analyser.',
      };
    }
    ...
    const systemPrompt = `Tu es un expert en analyse de journal personnel et spirituel. Tu analyses une note pour identifier les thèmes et donner un insight empathique.

${userLanguage === 'fr' ? `
IMPORTANT: Réponds UNIQUEMENT en français.

Ta mission:
1. Lire attentivement la note fournie par l'utilisateur
2. Identifier les émotions réelles exprimées
3. Détecter les thèmes principaux
4. Fournir un insight empathique, bienveillant et spirituel (2-3 phrases)

Format de réponse (JSON strict):
{
  "themes": ["Thème 1", "Thème 2"],
  "insight": "Contenu de l'insight ici..."
}
` : userLanguage === 'ar' ? `
مهم جداً: يجب أن ترد بالعربية فقط.
...
N'ajoute aucun texte avant ou après le JSON.`;
    ...
    const messages: ChatMessage[] = [
      { role: 'system', content: systemPrompt },
      { role: 'user', content: `Analyse cette note de journal :\n\n"${noteText}"` },
    ];
    ...
    if (error.message === 'OLLAMA_RATE_LIMIT' || error.isOllamaRateLimit || error.status === 429) {
      return {
        ...
        sentiment: 'neutral',
        themes: [],
        insight: 'L\'IA est temporairement surchargée. Veuillez réessayer dans quelques instants.',
      };
    }
