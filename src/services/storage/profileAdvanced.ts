/**
 * Service pour le profil avanc̩ (banni̬re, bio, statut)
 * Utilise Supabase Storage pour les images et user_metadata pour les donn̩es
 */
...
/**
 * R̩cup̬re le profil avanc̩ de l'utilisateur
 */
  ...
  try {
    ...
    if (APP_CONFIG.useSupabase && supabase) {
      try {
        ...
        console.error('Erreur r̩cup̩ration profil depuis Supabase:', error);
      }
    }
      ...
      try {
        ...
        if (!userError && user) {
          ...
          if (!updateError) {
            ...
            // La banni̬re est stock̩e dans user_metadata mais on peut aussi la mettre dans profiles
            // pour faciliter la r̩cup̩ration
            try {
              // V̩rifier si la table profiles existe et a une colonne pour la banni̬re
              // Pour l'instant, on garde la banni̬re dans user_metadata uniquement
              // car elle est charg̩e depuis user_metadata dans UserContext
            } catch (profileError) {
...
/**
 * S̩lectionne une image de banni̬re
 */
...
/**
 * Upload une banni̬re vers Supabase Storage
 */
  ...
  try {
    ...
    if (APP_CONFIG.useSupabase && supabase) {
      try {
        ...
        // Essayer d'obtenir l'utilisateur Supabase authentifi̩
        ...
        if (supabaseUser) {
          ...
          // Utilisateur non authentifi̩ mais userId fourni (depuis le contexte local)
          authenticatedUserId = userId;
        } else {
          // Utilisateur compl̬tement anonyme - g̩n̩rer un ID temporaire bas̩ sur le timestamp
          // Format: anonymous-{timestamp} pour permettre l'upload
          authenticatedUserId = `anonymous-${Date.now()}`;
        }

        // V̩rifier si le bucket existe en essayant de lister les buckets
        ...
        try {
          ...
          console.warn('Impossible de v̩rifier l\'existence du bucket, utilisation de l\'URI locale');
          return imageUri;
        }

        // Supprimer l'ancienne banni̬re si elle existe
        ...
        if (uploadError) {
          // Si l'erreur est li̩e au r̩seau ou au bucket, utiliser l'URI locale
          if (uploadError.message?.includes('Network') || 
              uploadError.message?.includes('bucket') ||
              uploadError.message?.includes('not found')) {
            console.warn('Erreur r̩seau ou bucket non disponible, utilisation de l\'URI locale:', uploadError.message);
            return imageUri;
          }
          throw uploadError;
        }
        ...
        // Si c'est une erreur r̩seau ou de connexion, utiliser l'URI locale
        if (error?.message?.includes('Network') || 
            error?.message?.includes('Failed to fetch') ||
            error?.code === 'PGRST301') {
          console.warn('Erreur r̩seau lors de l\'upload, utilisation de l\'URI locale:', error.message);
          return imageUri;
        }
        console.error('Erreur upload banni̬re vers Supabase:', error);
    ...
    // En cas d'erreur, retourner l'URI locale pour que l'utilisateur puisse quand m̻me utiliser l'image
    return imageUri;
  }
}
