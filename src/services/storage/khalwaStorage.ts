/**
 * Service de stockage des sessions Khalwa (Bayt An NÃ»r)
 * Sauvegarde et rÃ©cupÃ©ration des sessions depuis Supabase
 */
...
export interface KhalwaSessionData {
  ...
  duration: number; // en minutes (peut avoir des dÃ©cimales pour les secondes, ex: 0.5 = 30 secondes)
...
export async function saveKhalwaSession(
  ...
  // Tracker l'Ã©vÃ©nement analytics immÃ©diatement (mÃªme si hors ligne, il sera synchronisÃ© plus tard)
  ...
  if (!online || !supabase) {
    // Hors ligne ou Supabase non configurÃ© : ajouter Ã  la queue de synchronisation
  ...
  try {
    // VÃ©rifier d'abord si une session existe
    ...
    // CRITIQUE : RÃ©cupÃ©rer l'ID utilisateur directement depuis Supabase auth
    // Cela garantit que l'ID correspond bien Ã  auth.uid() utilisÃ© dans les politiques RLS
    ...
    // Utiliser l'ID Supabase authentifiÃ© (qui correspond Ã  auth.uid())
    const authenticatedUserId = supabaseUser.id;

    const { data, error } = await supabase
      .from('khalwa_sessions')
      .insert({
        user_id: authenticatedUserId, // Utiliser l'ID Supabase authentifiÃ©
    ...
    if (error) {
      // Erreur silencieuse en production
      if (error.code === '42501') {
        // Erreur RLS - VÃ©rifiez les politiques RLS
      }
      // Ajouter Ã  la queue pour synchronisation ultÃ©rieure
      await addToSyncQueue({
        type: 'khalwa_session',
    ...
    // Si sauvegarde Supabase rÃ©ussie, tracker l'Ã©vÃ©nement analytics
    ...
    // Si sauvegarde Supabase rÃ©ussie, on peut supprimer la version locale
 ...
 * PrioritÃ© : Supabase > AsyncStorage
 */
export async function loadKhalwaSessions(
  ...
  try {
    ...
    if (error) {
      ...
      // Erreur silencieuse en production
      // Essayer quand mÃªme AsyncStorage en fallback
      return loadKhalwaSessionsLocal(userId, limit);
    }
...
/**
 * Transformer une session de la base de donnÃ©es en objet KhalwaSessionData
 */
function transformSessionFromDB(dbSession: any): KhalwaSessionData {
  return {
    ...
    divineName: {
      ...
      meaning: '', // Pas stockÃ© en DB, sera rÃ©cupÃ©rÃ© depuis khalwaData si nÃ©cessaire
      meaningEn: '',
      description: ''
    },
    soundAmbiance: dbSession.sound_ambiance,
    duration: Number(dbSession.duration_minutes) || 0, // Convertir en nombre pour gÃ©rer les dÃ©cimales
...
export async function getKhalwaStats(userId: string): Promise<KhalwaStats | null> {
  ...
  try {
    ...
    // Si la fonction RPC n'existe pas ou Ã©choue, charger les sessions depuis Supabase et calculer
    ...
    // Autre erreur, essayer de charger depuis Supabase quand mÃªme
...
function calculateStatsFromSessions(sessions: KhalwaSessionData[]): KhalwaStats {
  ...
  // Calculer les sessions de cette semaine et ce mois (basÃ© sur les vraies dates)
  const now = new Date();
  ...
  // Calculer la sÃ©rie (basÃ©e sur les vraies dates, jours consÃ©cutifs)
  // Trier les sessions par date (plus rÃ©centes en premier pour calculer depuis aujourd'hui)
  ...
  // Parcourir depuis aujourd'hui vers le passÃ©
  sortedSessions.forEach((s) => {
    ...
    if (lastDate === null) {
      ...
      if (daysDiff === 1) {
        // Jour consÃ©cutif (hier)
        currentStreak++;
        lastDate = sessionDay;
      } else if (daysDiff === 0) {
        // MÃªme jour, ne pas compter comme nouveau jour mais garder la sÃ©rie
