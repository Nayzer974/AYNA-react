/**
 * Service pour gérer le stockage des conversations de chat
 */
...
export async function loadConversations(userId?: string, syncRemote: boolean = false): Promise<Conversation[]> {
  try {
    ...
    // Rien en local, tenter le cloud si demandé
    if (syncRemote && userId) {
      const remoteConversations = await fetchRemoteConversations(userId);
...
/**
 * Génère un titre automatique pour une conversation basé sur le premier message utilisateur
 */
